...
class TestReActAgent:
    ...
    def judge_scenario_execution(self, scenario_name, workspace_path, max_turns=10):
        ...
        initial_ci_result = execute_tool_in_workspace(
            workspace_path, "run_ci_pipeline", ""
        )
        ...
        if (
            isinstance(initial_ci_result, dict)
            and initial_ci_result.get("status") == "pass"
        ):
            scenario_data = {
        ...
        for turn in range(max_turns):
            ...
            if action_result.get("status") == "error":
                print(
                    f"DEBUG: Action failed with error: {action_result.get('error', 'Unknown error')}"
                )
                print(f"DEBUG: Full action_result: {action_result}")
                break
        ...
        # Debug: Print final step details
        if steps:
            last_step = steps[-1]
            print(f"\nDEBUG: Scenario failed after {len(steps)} steps")
            print(
                f"DEBUG: Last action: {last_step.get('action_result', {}).get('action', 'unknown')}"
            )
            print(
                f"DEBUG: Last action status: {last_step.get('action_result', {}).get('status', 'unknown')}"
            )
            if last_step.get("action_result", {}).get("status") == "error":
                print(
                    f"DEBUG: Last error: {last_step.get('action_result', {}).get('error', 'No error message')}"
                )
            print(
                f"DEBUG: Last observation: {last_step.get('observation', {}).get('observation', 'No observation')[:200]}"
            )
            print(
                f"DEBUG: Last CI status: {last_step.get('observation', {}).get('ci_status', 'unknown')}"
            )
    ...
    def assert_judge_quality(self, evaluation, min_score=60):
        ...
        ), f"Score too low: {evaluation['score']}/100. Feedback: {evaluation['feedback']}"
    ...
    def validate_act_output(self, action_result):
        ...
        assert status in [
            "success",
            "error",
        ], f"act() status should be 'success' or 'error', got '{status}'"

        # If success, should have result
        if status == "success":
            assert (
                "result" in action_result
            ), "Successful act() should have 'result' field"

    def validate_observe_output(self, observation):
        """Validate observe() method output"""
        self.validate_json_structure(
            observation, ["observation", "ci_status", "next_action_needed"], "observe()"
        )
        ...
        assert ci_status in [
            ...
            "fail",
            "unknown",
        ], f"ci_status should be 'pass', 'fail', or 'unknown', got '{ci_status}'"
    ...
    def test_simple_syntax_error_fix(self):
        """Test Case: Fix syntax error (missing colon) - Complete validation"""
        scenario_name = "seed_syntax"
        ...
        assert (
            workspace_result["status"] == "pass"
        ), f"Failed to create workspace: {workspace_result.get('error', 'Unknown error')}"
        ...
        print(
            f"✓ reason() quality: {reasoning_eval['score']}/100 - {reasoning_eval['feedback'][:50]}..."
        )
        ...
        print(
            f"✓ act() quality: {action_eval['score']}/100 - {action_eval['feedback'][:50]}..."
        )
        ...
        print(
            f"✓ observe() quality: {obs_eval['score']}/100 - {obs_eval['feedback'][:50]}..."
        )

        # Test full scenario execution with LLM judge
        judge_result = self.judge_scenario_execution(
            scenario_name, workspace_path, max_turns=max_steps
        )
        ...
        # Validate success
        assert (
            scenario_data["final_result"] == "success"
        ), f"Agent should fix {scenario_description} but got '{scenario_data['final_result']}'"
        ...
        print(
            f"✓ Full scenario quality: {full_evaluation['score']}/100 - {full_evaluation['feedback'][:50]}..."
        )

        # Efficiency check
        assert (
            scenario_data["total_turns"] <= max_steps
        ), f"Agent took {scenario_data['total_turns']} turns, expected <= {max_steps}"
    ...
    def test_multiple_syntax_errors_fix(self):
        """Test Case: Fix multiple syntax errors - Complete validation"""
        scenario_name = "seed_multi"
        max_steps = self.scenario_thresholds[scenario_name]
    ...
    def test_act_method_implemented(self):
        ...
        test_reasoning = {
            "reasoning": "test",
            "tool_call": {"tool": "test_tool", "input": "test_input"},
        }
        try:
