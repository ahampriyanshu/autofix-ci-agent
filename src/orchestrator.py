    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "final_ci_status": ci_result.get("data", {}).get(
            ...
            "error": (
                None
                if success
                else f"Agent result: {agent_result.get('status', 'unknown')}, CI status: {ci_result.get('status', 'unknown')}, Final CI: {ci_result.get('data', {}).get('overall_status', 'unknown')}"
            ),
        }
...
def create_error_workspace(seed_name):
    try:
        ...
        return {
            "status": "pass",
            "data": {"workspace_path": str(workspace_path), "error_info": error_info},
        }
